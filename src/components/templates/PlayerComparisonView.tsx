...
function PlayerAvatar({ src, accent, label, isWide }: { src: string; accent: string; label: string; isWide: boolean }) {
  if (!src) return null;
  return <div className={`${isWide ? 'w-20 h-20' : 'w-24 h-24'} rounded-full overflow-hidden border-2 flex-shrink-0 bg-black/30 shadow-xl`} style={{ borderColor: `${accent}80` }}><img src={src} alt={label} className="w-full h-full object-contain object-bottom" crossOrigin={src.startsWith('http') ? 'anonymous' : undefined} referrerPolicy="no-referrer" /></div>;
}
...
export const PlayerComparisonView: React.FC<TemplateProps> = ({ project }) => {
  ...
  return (
    <div className={`relative z-20 w-full h-full flex flex-col justify-between ${isWide ? 'p-8' : 'p-14 md:p-16'} select-none`}>
      <EditorialHeader categoryBadge="Head-to-Head • Analytical Comparison" title={`${data.player1.name} VS ${data.player2.name}`} subtitle={subtitle} theme={theme} fontDisplay={fontDisplay} layout={advancedLayout} />
      ...
      <div className={`flex-1 ${isWide ? 'my-3 gap-3' : 'my-6 gap-6'} flex flex-col justify-center max-w-[2000px] mx-auto w-full`}>
        ...
        <div className={`rounded-2xl ${isWide ? 'p-4 gap-2' : 'p-6 gap-4'} border backdrop-blur-md flex flex-col shadow-2xl`} style={{ backgroundColor: 'rgba(10, 14, 26, 0.9)', borderColor: 'rgba(255, 255, 255, 0.08)' }}>
          {metrics.map((metric) => {
        ...
        {data.verdictText && (
          <div className="rounded-2xl p-5 border backdrop-blur-md flex items-start gap-4 shadow-xl" style={{ backgroundColor: 'rgba(10, 14, 26, 0.85)', borderColor: `${theme.primaryAccent}30` }}>
            ...
            <div><div className="text-xs font-black tracking-widest uppercase mb-1" style={{ color: theme.primaryAccent }}>{data.verdictTitle || 'ANALYTICAL VERDICT'}</div><p className="text-white font-medium leading-relaxed" style={{ fontSize: scaledTemplateFontSize(19, advancedLayout, 'verdict', 14, 26) }}>{data.verdictText}</p></div>
          </div>
        )}
      </div>
