  ...
  return (
    <div className={`relative z-20 w-full h-full flex flex-col justify-between ${isWide ? 'p-5' : 'p-9 md:p-10'} select-none`}>
      ...
      <div className={`flex-1 grid grid-cols-12 ${isWide ? 'gap-3 my-1.5' : 'gap-5 my-3'} items-center min-h-0`}>
        <div className={`col-span-8 flex flex-col ${isWide ? 'gap-2.5' : 'gap-3'} min-h-0`}>
          {scoutingHeadline && (
            <div className="font-black uppercase tracking-tight leading-[1.02] pr-2" style={{ fontFamily: fontDisplay, color: theme.primaryAccent, fontSize: scaledTemplateFontSize(isWide ? 18 : 22, advancedLayout, 'subtitle', 14, 28) }}>
              {scoutingHeadline}
            </div>
          )}

          {advancedLayout.visibleBlocks.tacticalProfile !== false && profile.tacticalProfile && (
            <div className={`rounded-2xl ${isWide ? 'px-4 py-3' : 'px-4 py-3.5'} border backdrop-blur-md shadow-xl`} style={{ backgroundColor: 'rgba(8, 12, 22, 0.85)', borderColor: `${theme.primaryAccent}25` }}>
          ...
          {advancedLayout.visibleBlocks.stats !== false && visibleStats.length > 0 && (
            <div className="space-y-1.5">
              ...
              <div className={`grid ${statColumns} gap-2`}>
                {visibleStats.map((st) => <EditorialStatCard key={st.id} stat={st} theme={theme} fontDisplay={fontDisplay} className={statCardClass} layout={advancedLayout} />)}
              </div>
            </div>
          )}

          {(advancedLayout.visibleBlocks.strengths !== false || advancedLayout.visibleBlocks.development !== false) && (
            <div className={`rounded-2xl ${isWide ? 'px-4 py-3' : 'px-4 py-3.5'} border backdrop-blur-md grid grid-cols-2 ${isWide ? 'gap-4' : 'gap-5'} shadow-2xl`} style={{ backgroundColor: 'rgba(8, 12, 22, 0.90)', borderColor: 'rgba(255, 255, 255, 0.12)' }}>
          ...
          {advancedLayout.visibleBlocks.summary !== false && profile.summary && (
            <div className={`rounded-2xl ${isWide ? 'px-4 py-3' : 'px-4 py-3.5'} border backdrop-blur-md relative overflow-hidden shadow-2xl`} style={{ backgroundColor: 'rgba(8, 12, 22, 0.92)', borderColor: `${theme.primaryAccent}40`, boxShadow: `0 20px 40px -10px rgba(0,0,0,0.85), inset 4px 0 0 0 ${theme.primaryAccent}` }}>
