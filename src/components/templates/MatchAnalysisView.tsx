      ...
      <div>
        <div className="flex items-center gap-3 mb-2">
          <span
            ...
            style={{ backgroundColor: `${theme.primaryAccent}20`, borderColor: `${theme.primaryAccent}50`, color: theme.primaryAccent }}
          >
        ...
        {hasScorers && (
          <div className={`grid grid-cols-2 ${isWide ? 'gap-6 mt-1' : 'gap-10 mt-2'} w-full text-neutral-300 font-semibold`}>
            <div className={`flex items-start gap-2 ${isWide ? 'text-[14px]' : 'text-[16px]'}`}>
              {scorersTeam1.length > 0 && <><IconBallFootball size={18} className="mt-0.5 flex-shrink-0" style={{ color: theme.primaryAccent }} /><span>{scorersTeam1.join(', ')}</span></>}
            </div>
            <div className={`flex items-start justify-end gap-2 text-right ${isWide ? 'text-[14px]' : 'text-[16px]'}`}>
              {scorersTeam2.length > 0 && <><span>{scorersTeam2.join(', ')}</span><IconBallFootball size={18} className="mt-0.5 flex-shrink-0" style={{ color: team2Accent }} /></>}
            </div>
          </div>
        )}
      </div>
      ...
      <div className={`flex-1 w-full ${isWide ? 'my-4' : 'my-6'} flex flex-col ${isWide ? 'gap-3' : 'gap-5'} justify-center`}>
        {data.tacticalSummary?.trim() && (
          <div className={`rounded-2xl ${isWide ? 'p-4' : 'p-6'} border backdrop-blur-md shadow-xl`} style={{ backgroundColor: 'rgba(10, 14, 26, 0.85)', borderColor: `${theme.primaryAccent}30` }}>
            <div className="text-xs font-black tracking-widest uppercase text-neutral-400 mb-1">Match Overview</div>
            <p className={`${isWide ? 'text-[17px]' : 'text-[21px]'} text-white font-medium leading-relaxed`}>{data.tacticalSummary}</p>
          </div>
        )}

        {stats.length > 0 && (
          <div className={`rounded-2xl ${isWide ? 'p-4 gap-3' : 'p-6 gap-4'} border backdrop-blur-md flex flex-col shadow-xl`} style={{ backgroundColor: 'rgba(10, 14, 26, 0.85)', borderColor: 'rgba(255, 255, 255, 0.08)' }}>
            <div className="text-xs font-black tracking-widest uppercase text-neutral-400">Key Match Metrics</div>
            <div className={`grid ${stats.length > 2 && isWide ? 'grid-cols-2' : 'grid-cols-1'} gap-x-6 gap-y-3`}>
                ...
                return (
                  <div key={`${stat.label}-${index}`} className="flex flex-col gap-1.5">
                    <div className={`${isWide ? 'text-[14px]' : 'text-[16px]'} flex items-center justify-between font-bold`}>
                      ...
                      <span className="text-neutral-400 uppercase tracking-wider text-xs font-black text-center px-3">{stat.label}</span>
                      <span style={{ color: team2Accent }}>{stat.val2}</span>
                    </div>
                    <div className="h-3 rounded-full bg-neutral-800 overflow-hidden flex">
                      <div className="h-full rounded-l-full" style={{ width: `${player1Share}%`, backgroundColor: theme.primaryAccent }} />
                      <div className="h-full rounded-r-full" style={{ width: `${100 - player1Share}%`, backgroundColor: team2Accent }} />
                    </div>
                  </div>
                );
              })}
        ...
        <div className={`grid ${takeaways.length > 0 && hasPerformer && isWide ? 'grid-cols-[1.4fr_.8fr]' : 'grid-cols-1'} gap-4`}>
          {takeaways.length > 0 && (
            <div className={`rounded-2xl ${isWide ? 'p-4' : 'p-6'} border backdrop-blur-md shadow-xl`} style={{ backgroundColor: 'rgba(10, 14, 26, 0.85)', borderColor: 'rgba(255, 255, 255, 0.08)' }}>
              <div className="text-xs font-black tracking-widest uppercase text-neutral-400 mb-2.5">Key Takeaways</div>
              <div className="flex flex-col gap-2">
                {takeaways.map((takeaway, idx) => (
                  <div key={idx} className={`flex items-start gap-2.5 ${isWide ? 'text-[15px]' : 'text-[17px]'} text-neutral-200 font-semibold`}>
                    <span className="w-2 h-2 rounded-full mt-2 flex-shrink-0" style={{ backgroundColor: theme.primaryAccent }} />
                    <span>{takeaway}</span>
                  </div>
                ))}
              </div>
          ...
          {hasPerformer && (
            <div className={`rounded-2xl ${isWide ? 'p-4' : 'p-4'} border backdrop-blur-md flex items-center justify-between gap-5 shadow-xl`} style={{ backgroundColor: 'rgba(10, 14, 26, 0.9)', borderColor: `${theme.primaryAccent}40` }}>
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center border flex-shrink-0" style={{ backgroundColor: `${theme.primaryAccent}15`, borderColor: `${theme.primaryAccent}35`, color: theme.primaryAccent }}><IconStar size={22} /></div>
                <div className="min-w-0">
                  <div className="text-[11px] font-black uppercase tracking-widest text-neutral-400">{data.performerTitle?.trim() || 'PLAYER OF THE MATCH'}</div>
                  <div className={`${isWide ? 'text-[22px]' : 'text-[26px]'} font-black uppercase tracking-tight text-white leading-none`} style={{ fontFamily: fontDisplay }}>{data.performerName}</div>
                </div>
              </div>
              {data.performerNote?.trim() && <div className={`${isWide ? 'text-[13px]' : 'text-[15px]'} text-right font-bold text-neutral-300 max-w-[48%]`}>{data.performerNote}</div>}
            </div>
          )}
        </div>
      </div>
